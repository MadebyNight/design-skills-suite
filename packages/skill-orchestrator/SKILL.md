...
用户询问“套件能做什么、怎么用、该选哪个 Skill”时，先介绍其用途：将支付宝活动需求转换为可评审、可交接的受控视觉方案，减少素材生成、适配和页面组装的重复工作。仅咨询时不创建请求、不运行批次、不探测图片服务。

按需求推荐入口：首页用 `skill-alipay-home`，落地页用 `skill-alipay-landing`，单独生图用 `skill-image-generate`，已有图片处理用 OpenPhoto；自动生成并回填素材、多页交付、失败恢复或离线联动用本编排器，单页素材链路也可编排。不要把所有独立任务都强制转为批次。

示例：“设计一个首页和两个活动落地页，主题分别为开学季与演唱会，自动生成并回填素材，提供离线联动预览。”从上下文提取页面类型、主题和数量，可选补充受众、品牌要求、文案与参考图。由 Agent 整理结构化请求，仅询问影响执行的缺项；CLI 不直接解析自然语言。需求明确后沿用执行流程，不重复欢迎或介绍。

执行前区分本地测试图片与真实 provider；真实生图可能产生费用，按当前会话授权执行。交付时提供结果返回的 `index.html` 实际路径，说明成功、失败、待补素材及配置建议的位置。原型和配置不能直接发布，真实资源、生产路由和上线由业务系统负责。详细步骤见[使用指南](../../docs/usage/design-skills-user-guide.md#新用户使用流程)。命令帮助为 `npm run batch -- --help`，不创建批次。
