...
这套工具帮助你把活动主题变成可评审、可交接的支付宝页面视觉方案，减少素材制作、尺寸适配和页面组装的重复工作。

| 你想做什么 | 使用入口 |
|---|---|
| 设计一个首页 | `skill-alipay-home` |
| 设计一个活动落地页 | `skill-alipay-landing` |
| 单独生成图片 | `skill-image-generate` |
| 裁切、缩放或导出已有图片 | OpenPhoto |
| 自动生成并回填页面素材、多页设计或离线联动预览 | `skill-orchestrator`（也可处理单页素材链路） |

在能访问本仓库的 Agent 会话中，可以直接这样说：

> 请读取本仓库对应的 SKILL.md，帮我设计一个开学季支付宝活动落地页，面向大学生，绿色清新风格，自动生成并回填素材，交付离线预览。

> 请先介绍这个套件能做什么，以及设计一个首页需要我提供哪些信息，暂时不要生成。

页面需求至少说明**页面类型、主题和数量**，也可以附上受众、品牌要求和参考图。Agent 根据已加载的 Skill 整理结构化请求；CLI 本身接收 JSON，不直接解析自然语言。首次环境准备见下文；完整步骤见[新用户使用流程](docs/usage/design-skills-user-guide.md#新用户使用流程)。

单独调用页面 Skill 时，缺图会返回待生成素材请求；需要完整生图与回填时使用编排器。交付后先打开返回的 `index.html`（批量任务）或 `prototype.html`（单页设计包），再阅读配置建议。
